...
The existing kernel provided fixed membership, majority quorum calculation, sequential transition indexes, epoch checking, root chaining, duplicate-voter deduplication, commit receipts, and deterministic replay-root calculation.

It did not provide a per-voter lock preventing one member from voting for two conflicting transitions at the same epoch/index; an independently verifiable quorum certificate; a membership hash bound into the proposal/vote/receipt; receipt-integrity verification independent of the committing kernel object; deterministic crash recovery from transition + receipt sequences; a safe membership-change protocol; durable WAL/fsync semantics; transport or leader election; or cryptographic member signatures.
...
The existing directory provided coordinate registration, manifestation upsert/detach, generation checking, sequential apply index, and a deterministic directory root.
...
A planner is not an actuator and an intended state is not an observed operational state.

## 3. Engineering layer built in this pass

### 3.1 ToT safety kernel R1

The upgraded kernel retains the project invariant that zero is not a live address/state value while adding a fixed-membership safety envelope.

Each transition now carries log `index`, `epoch`, member `actor`, `command` and payload, `previous_root`, and `membership_hash`. The membership hash is SHA-256 over the sorted fixed member set and epoch, preventing a transition from another membership/epoch from silently entering the current log.

For each `(epoch, index, voter)` the kernel records the digest that member voted for. A second vote by the same member for another digest at the same position raises `VOTER_EQUIVOCATION`.
...
A commit now materializes a `QuorumCertificate` containing epoch, index, transition digest, membership hash, sorted voter list, and certificate hash. The certificate can be verified independently against a member list.

The commit receipt binds transition digest, previous committed root, new committed root, membership hash, quorum certificate, and receipt hash. The committed root is chained from `previous_root + transition_digest + quorum_certificate_hash`. Tampering with the receipt hash or certificate is therefore detected by the verifier.

`recover()` reconstructs the kernel from a transition/receipt sequence while checking equal sequence lengths, contiguous indices, previous-root continuity, receipt/certificate validity, and membership hash. This provides deterministic logical crash recovery from already-durable records. It does **not** itself make those records durable on disk.

`reconfigure_membership()` fails closed with `MEMBERSHIP_CHANGE_PROTOCOL_NOT_IMPLEMENTED`. This is intentional evidence, not an omission hidden by an architecture diagram. Safe membership transition requires a separate protocol.
...
When configured with the membership list, each applied transition must pass `ToTSafetyKernel.verify_receipt()` before changing directory state. The directory therefore no longer assumes that a `CommitReceipt` object is trustworthy merely because it has the right fields.

A replica only applies `applied_index + 1`. Gaps fail with `DIRECTORY_REPLAY_GAP`. Receipt roots must also form a continuous commit chain. Divergent commit ancestry fails with `DIRECTORY_COMMIT_CHAIN_DIVERGENCE`.

Manifestation generations must strictly increase. Replaying or overwriting the same manifestation at an equal or older generation fails with `STALE_GENERATION`. This prevents same-generation conflicting overwrite.

`sync_from()` compares all common receipt hashes before accepting the missing suffix. If a supposedly common prefix differs, synchronization fails with `DIRECTORY_REPLICA_DIVERGENCE`. A clean suffix can be replayed and both replicas converge on the same deterministic directory root.
...
It consumes distinct maps for desired manifestation state and observed manifestation state. This prevents intended configuration from being treated as proof of applied operational state.

The planner emits `MATERIALISE` when desired state is absent, `REPLACE` when endpoint/generation/state differs, and `DETACH` when observed state has no desired object. An observed generation ahead of desired fails closed because blindly replacing newer observed state with an older intent would be unsafe.

`reconcile_once()` now calls an actuator for each action. The reconciler itself remains transport/device agnostic.

Every action receives a deterministic idempotency key derived from reconcile identity and canonical action content. An actuator can therefore deduplicate retries.

Every action returns an `ObservedManifestation` or raises an error. The reconciler records `ActionReceipt` objects and returns a `ReconcileReceipt` containing reconcile ID, desired root, observed-before root, observed-after root, action receipts, convergence flag, status, and receipt hash.
...
Command executed locally against the exact engineering files prepared for deployment:
...
Observed summary: 8 scenarios, 8 passed, 0 failed.
...
RFC 8342 distinguishes intended configuration from operational state and specifically allows clients to determine how much intended configuration is actually in use by comparing against operational state. This supports the decision in Report 03 not to treat desired manifestation state as proof of applied state.
...
**ToT:** advanced from “majority votes were counted” to “conflicting per-position votes are locked, certified, independently verifiable, chain-bound and recoverable.” Evidence level: **EXECUTED LOCALLY / FAULT-INJECTED**.

**Coordinate directory:** advanced from “one deterministic in-memory directory” to “replicas can verify committed input, compare common history, replay a committed suffix, detect divergence, and converge to the same root.” Evidence level: **EXECUTED LOCALLY across independent Python directory objects**.

**Layer‑2 reconciler:** advanced from “planner” to “controller transaction with actuator invocation, deterministic retry identity, readback, partial failure and convergence receipt.” Evidence level: **EXECUTED LOCALLY with fault-injected fake actuators**.

**Evidence discipline:** advanced because failure is now an explicit state in the receipts. A failed actuator or unsupported membership transition cannot be promoted into success by the report.

## 8. What remains unproven, why, why the reason remains, and what architecture is absent

### 8.1 Multi-host distributed consensus

**Unproven:** independent hosts safely committing under process crashes, delay, duplication, reordering, network partitions, restart, and concurrent proposals.
...
**Why the underlying problem remains:** a majority rule over one member set cannot simply switch to another set without defining transition overlap and commit rules. Naive reconfiguration can create two valid majorities on different configurations.

**Missing architecture:** joint membership/joint consensus or equivalent configuration-epoch protocol, configuration certificates, admission/removal proof, recovery rules, and tests across overlapping/non-overlapping sets.
...
**Why still unaddressed:** this engineering layer is deliberately non-Byzantine. Implementing PBFT/HotStuff-style machinery would be a different trust model rather than “finishing” this one.

**Missing architecture if Byzantine tolerance is later required:** per-member cryptographic identity, signed votes, replay protection/nonces, authenticated transport, Byzantine quorum thresholds, view change, quarantine/revocation semantics, key rotation and compromise recovery.

### 8.4 Linearizable distributed coordinate service

**Unproven:** arbitrary clients receiving linearizable reads/writes from a live multi-host coordinate service.
...
**Unproven:** the stronger proposition that a virtual coordinate/node can never disappear once sufficiently replicated.
...
**Unproven:** reconciliation changing a real Ethernet bridge, VLAN, host interface, virtual switch, or NIC.
...
**Unproven:** mathematical proof that every reachable implementation state satisfies intended safety invariants.
...
**Why unproven:** Report 03 is a correctness/falsification pass, not a production-scale benchmark.
...
6. Add YANG/RFC 8343/8345 projection if external management interoperability is required.
7. Implement an actual bridging dataplane only if the product claim is “network bridge,” rather than “L2 manifestation reconciler.”
