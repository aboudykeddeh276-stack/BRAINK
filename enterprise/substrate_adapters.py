...
class SQLiteJsonAdapter:
    adapter_id="adapter://sqlite/json"
    schemes=("sqlite://",)
    capabilities=("READ","WRITE","PROBE","ATOMIC")
    def _parts(self,backing):
        spec=backing.removeprefix("sqlite://")
        path,_,table=spec.partition("#")
        return Path(path), table or "objects"
    def _conn(self,backing):
        path,table=self._parts(backing); path.parent.mkdir(parents=True,exist_ok=True)
        c=sqlite3.connect(path); c.execute(f"CREATE TABLE IF NOT EXISTS {table}(logical TEXT PRIMARY KEY,payload TEXT NOT NULL,value_hash TEXT NOT NULL)")
        return c,table
    def probe(self,backing):
        c,t=self._conn(backing); c.close(); return {"status":"READY","table":t}
    def apply(self,backing,logical,operation,payload=None):
        c,t=self._conn(backing)
        try:
            if operation=="WRITE":
                raw=json.dumps(payload,sort_keys=True,separators=(",",":")); h=hashlib.sha256(raw.encode()).hexdigest()
                c.execute(f"INSERT OR REPLACE INTO {t} VALUES(?,?,?)",(logical,raw,h)); c.commit()
                return {"status":"COMMITTED","value_hash":h,"table":t}
            if operation=="READ":
                r=c.execute(f"SELECT payload,value_hash FROM {t} WHERE logical=?",(logical,)).fetchone()
                return {"status":"HOLE"} if not r else {"status":"READ","value":json.loads(r[0]),"value_hash":r[1]}
            return {"status":"UNSUPPORTED_OPERATION","operation":operation}
        finally: c.close()

class MemoryAdapter:
    adapter_id="adapter://memory/json"
    schemes=("mem://",)
    ...
    def apply(self,backing,logical,operation,payload=None):
        k=(backing,logical)
        if operation=="WRITE":
            self.data[k]=payload; return {"status":"COMMITTED","value_hash":digest(payload)}
        if operation=="READ":
            return {"status":"HOLE"} if k not in self.data else {"status":"READ","value":self.data[k],"value_hash":digest(self.data[k])}
...
class CapabilityRegistry:
    def __init__(self):
        self.adapters={}
        self.descriptors={}
    def register(self,adapter,priority=100):
        self.adapters[adapter.adapter_id]=adapter
        self.descriptors[adapter.adapter_id]=AdapterDescriptor(adapter.adapter_id,adapter.schemes,adapter.capabilities,priority)
    def discover(self,backing,operation):
        candidates=[]
        for aid,d in self.descriptors.items():
            if operation not in d.capabilities: continue
            if not any(backing.startswith(s) for s in d.schemes): continue
        ...
        _,aid,probe=sorted(candidates,key=lambda x:(x[0],x[1]))[0]
        return {"status":"RESOLVED","adapter_id":aid,"probe":probe}
    def invoke(self,adapter_id,**kwargs):
        return self.adapters[adapter_id].apply(**kwargs)
