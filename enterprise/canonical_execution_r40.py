...
def resource_requirement(body: dict[str, Any]) -> ResourceRequirement:
    if not isinstance(body, dict): raise ValueError("RESOURCE_REQUIREMENT_REQUIRED")
...
class CanonicalExecutionR40:
    """Thin coordinator: resident components retain mutation authority."""

    def __init__(self, runtime_host: Any, state_root: str | Path, fabric_admission: Any | None = None):
        self.host = runtime_host; self.state_root = Path(state_root); self.state_root.mkdir(parents=True, exist_ok=True)
    ...
    def execute(self, command: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            classified = self.data_classes.classify(source=command.get("source", ""), data_class=command.get("data_class", ""),
                                                     payload=command.get("payload", {}), schema_version=command.get("schema_version", "1"),
                                                     sector=command.get("sector"))
        except Exception as exc: return self._block("DATA_CLASS", stages, str(exc))
        stages += ["CLASSIFIED", "SCHEMA_VALIDATED"]
        authority = str(command.get("authority", "")).strip()
        if not authority.startswith("authority://"): return self._block("AUTHORITY", stages, "AUTHORITY_URI_REQUIRED")
        stages.append("AUTHORITY_VALIDATED")
        illlm_request = command.get("illlm")
        if not isinstance(illlm_request, dict): return self._block("IL_LLM_REQUEST", stages)
        try: normalized, binding = self.illlm.resolve(illlm_request)
        except Exception as exc: return self._block("IL_LLM_RESOLUTION", stages, str(exc))
        ...
        envelope = None; node_created = False; vfs_root = None

        if binding.intent == "computer.instantiate":
            if not classified.node_eligible: return self._block("DATA_CLASS_NOT_NODE_ELIGIBLE", stages, classified.to_dict())
            try: self.host.resolve(normalized["lineage"])
            except Exception as exc: return self._block("PARENT_NODE_RESOLUTION", stages, str(exc))
            child_id = normalized["child_id"]; child_lineage = normalized["lineage"].rstrip("/") + "/" + child_id
            try:
                ...
                return {"status": "ROUTED_EXISTING_NODE", "stages": stages + ["EXISTING_NODE_RESOLVED", "FIXED_POINT", "QUIESCED"],
                        "data_class": classified.to_dict(), "node": self.host.snapshot(existing), "observation": observation}
            except Exception: pass
            try: envelope = self.scheduler.allocate(child_id, resource_requirement(command.get("resources")))
            except Exception as exc: return self._block("RESOURCE_SCHEDULER", stages, str(exc))
            stages.append("RESOURCE_ENVELOPE_GRANTED")
            try: result = self.illlm.execute(illlm_request, self.host)
            except Exception as exc:
                self.scheduler.release(child_id); return self._block("RUNTIME_EXECUTION", stages, str(exc))
            node_created = True; stages += ["NODE_MATERIALIZED", "RUNTIME_EXECUTED"]
            ...
            identity_write = vfs.write(child_id, "identity.json", {"node_id": child_id, "logical_identity": classified.logical_identity,
            ...
            if not target_exists: return self._block("EXISTING_NODE_REQUIRED", stages, normalized["lineage"])
            ...
            try: result = self.illlm.execute(illlm_request, self.host)
            except Exception as exc: return self._block("RUNTIME_EXECUTION", stages, str(exc))
            stages += ["EXISTING_NODE_RESOLVED", "RUNTIME_EXECUTED"]

        post = self.host.snapshot(target_node)
        if not post.get("ledger_verified"): return self._block("LOCAL_LEDGER_VERIFICATION", stages)
        ...
        capabilities = sorted(set(map(str, command.get("capabilities", []))))
        network_id = str(command.get("network_id", f"network://local/{node_id}"))

        mesh_result = None; server_result = None; subscription_result = None
        if self.fabric_admission is not None:
            if capabilities:
                mesh_result = self.fabric_admission.register_local_node(node_id=node_id, lineage=list(target_node.identity.lineage),
                                                                        state_root=post["state_root"], capabilities=capabilities, authority=authority,
                                                                        network_id=network_id, vfs_root=vfs_root or f"vfs://node/{node_id}")
                if mesh_result.get("status") == "MESH_REGISTERED": stages.append("MESH_REGISTERED")
            else:
                mesh_result = {"status": "BLOCKED:CAPABILITY_MANIFEST_REQUIRED", "node_id": node_id}
            if command.get("server_registration"):
                server_result = self.fabric_admission.register_server(command["server_registration"])
                if server_result.get("status") == "SERVER_REGISTERED": stages.append("SERVER_REGISTERED")
            ...
            if topics:
                ...
                if subscription_result.get("status") == "SUBSCRIBED": stages.append("SUBSCRIBED")
        else:
        ...
        if command.get("global_delta") is not None:
            ...
            try:
                global_result = self.global_knowledge.apply_delta(source_node=node_id, source_event=ledger_ref,
                    data_class=classified.data_class, previous_version=int(delta.get("previous_version", snap["version"])),
                    relation_delta=delta.get("relation_delta", {}), provenance={"data_id": classified.data_id, "command_root": observation["command_root"]},
                    authority=authority, validation={"status": "VALIDATED", "local_ledger_verified": True}, ledger_reference=ledger_ref)
            except Exception as exc: return self._block("GLOBAL_IL_LLM_DELTA", stages, str(exc))
            ...
            if check != global_result["version"]: return self._block("GLOBAL_VERSION_READBACK", stages)
        ...
        return {
            "status": "EXECUTED_DISTRIBUTED_REGISTERED" if distributed_ok else "EXECUTED_LOCAL_VERIFIED",
            "stages": stages, "observation": observation, "data_class": classified.to_dict(),
            "il_llm": {"normalized": normalized, "binding": asdict(binding)}, "result": result,
            "readback": self.host.snapshot(target_node), "ledger_reference": ledger_ref, "global_delta": global_result,
            "mesh_state": mesh_result, "server_state": server_result or {"status": "BLOCKED:SERVER_REGISTRATION_NOT_REQUESTED"},
