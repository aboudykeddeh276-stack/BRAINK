...
class CapabilityDeploymentRuntime:
    ...
    def bind(self,capability,implementation_ref,evidence_ref=None,state="VERIFIED"):
        p=self.capabilities[capability];self.capabilities[capability]=Capability(p.name,p.gene,p.server_family,p.owner_sector,state,implementation_ref,evidence_ref)
    def compile(self,undertaking:str,scale="SMALL"):
        ...
        body={"schema":"braink.capability-deployment.r18/v1","undertaking":undertaking,"generated_ns":time.time_ns(),"genome":{"id":genome.genome_id,"genes":list(genome.genes),"capability_root":genome.capability_root},"room_root":room.room_root,"server_sets":[{"family":s.family,"replicas":s.replicas,"services":list(s.services),"dependencies":list(s.dependencies),"config_root":s.config_root} for s in room.servers],"requirements":required}
