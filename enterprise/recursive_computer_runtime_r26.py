...
class RecursiveComputer:
    ...
    def _reconcile_runtime(self, payload):
        ...
        if rb.get('status') != 'READ': return {'status':'BLOCKED','reason':'COMMITTED_STATE_UNREADABLE','subject':subject,'readback':rb}
        snap = rb['value']
        if snap.get('constructor') != self.CONSTRUCTOR_ID or snap.get('identity',{}).get('computer_id') != self.identity.computer_id: return {'status':'BLOCKED','reason':'IDENTITY_OR_CONSTRUCTOR_MISMATCH','subject':subject}
    ...
    def reconcile_once(self): return self.runtime.continuation_tick()
    ...
    def _persist(self, operation):
        ...
        if r.get('status')=='CONFLICT': self.runtime.observe('runtime://computer',f'computer://{self.identity.computer_id}/state','CONTRADICTION',r); raise RuntimeError('STALE_STATE_CONFLICT')
        if r.get('status')!='COMMITTED': raise RuntimeError(f'PERSIST_FAILED:{r}')
        self._expected_state_hash=r['value_hash']; rb=self.runtime.route(f'computer://{self.identity.computer_id}/state',self.state_backing,'READ')
        if rb.get('status')!='READ' or rb.get('value')!=snap: raise RuntimeError('READBACK_MISMATCH')
        self._record_event(operation=operation,input_state=snap,output_state=rb['value'],proof={'readback_equal':True,'value_hash':rb['value_hash']},rollback={'checkpoint':self.runtime.checkpoint()}); return rb
    @classmethod
    def restore(cls, state_root, *, recursive=False):
        ...
        c._record_event(operation='WARM_BOOT_TREE_RESTORE' if recursive else 'WARM_BOOT_RESTORE',input_state=snap,output_state=rb['value'],proof={'readback_equal':True,'restored':True,'recursive':recursive,'descendants':sorted(c.children)},rollback={'checkpoint':c.runtime.checkpoint()}); return c
    @classmethod
    ...
    def _refresh_constructor_view(self):
        ...
        self.state=_copy(snap.get('state',{})); self.memory=_copy(snap.get('memory',{})); self._expected_state_hash=rb['value_hash']; committed_ids=set(snap.get('children',[]))
        for child_id in committed_ids:
            child_root=self.state_root/'descendants'/child_id; child_state=child_root/'computer.json'
        ...
        self._committed_child_ids=committed_ids; self.children={k:v for k,v in self.children.items() if k in committed_ids}; self.ledger,self._expected_ledger_hash=self._load_committed_ledger(); return snap
    ...
    def _quarantine_uncommitted_child(self, child, reason):
        ...
        self.runtime.observe('runtime://computer',f'computer://{self.identity.computer_id}/descendants/{child.identity.computer_id}','CONTRADICTION',receipt); return {'status':'QUARANTINED','path':str(target),'child_state_root':child_root,'receipt_hash':rr['value_hash']}
