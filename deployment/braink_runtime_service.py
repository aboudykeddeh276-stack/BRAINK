...
class Store:
    ...
    def operate(self, action, p):
        ...
        dispatch={
            ...
            "customer_file.update_service_state": lambda: op.customers.update_service_state(p["customer_id"],p.get("patch",{}),p["authority"]),
            "customer_file.attach_document": lambda: op.customers.attach_document(p["customer_id"],p["address"],p.get("purpose","GENERAL"),p["authority"]),
            "customer_file.record_communication": lambda: op.customers.record_communication(p["customer_id"],p["channel"],p["direction"],p["summary"],p["authority"]),
            "customer_file.record_billing": lambda: op.customers.record_billing_event(p["customer_id"],p["event_type"],p["amount_minor"],p["currency"],p.get("external_ref"),p["authority"]),
            "customer_file.export": lambda: op.customers.export_manifest(p["customer_id"],p["export_id"],p["authority"]),
            "customer_file.close": lambda: op.customers.close_customer_file(p["customer_id"],p["disposition"],p.get("retention",{}),p["authority"]),
