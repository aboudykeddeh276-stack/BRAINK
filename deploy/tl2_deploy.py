    ...
    while time.time() < end:
        try:
            ...
            with urllib.request.urlopen(req, timeout=3) as response:
                body = response.read()
                return {"url": url, "status": response.status, "bytes": len(body), "sha256": _sha(body), "ok": response.status < 400}
...
def _spawn_supervisor(host: str, port: int) -> subprocess.Popen:
    ...
    return subprocess.Popen([
        sys.executable, str(MODULES / "service_supervisor.py"), "--host", host, "--port", str(port),
        "--max-restarts", "5", "--restart-window", "60",
    ], cwd=BASE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True, env=env)
...
def _wait_for_current_generation(supervisor: subprocess.Popen, timeout: int = 20) -> dict:
    end = time.time() + timeout
    last = {"observedState": "UNOBSERVED"}
    while time.time() < end and supervisor.poll() is None:
        last = _read_supervisor_state()
        if (
            last.get("supervisorPid") == supervisor.pid
            and last.get("observedState") == "RUNNING"
            and int(last.get("generation") or 0) >= 1
            and _pid_alive(last.get("childPid"))
        ):
            return last
        time.sleep(0.2)
    return last
...
def deploy(daemon: bool) -> int:
    ...
    if not host:
        result = {"status": "BLOCKED_TL2_ACTUATOR", "promotion": None, "identity": TL2_ID, "source_object": SOURCE_ID, "service": SERVICE_ID, "runtime": RUNTIME_ID, "reason": "No KEX_TL2_ADDRESS or observed TL2/tunnel interface address", "unrelated_blocks_excluded": ["PUBLIC_DNS", "PUBLIC_TLS", "DRIVE_WRITEBACK", "BITCOIN_IBD", "BRAINK_FULL_MIGRATION"]}
        _write_report(result); _proof("TL2_BIND_BLOCKED", result); print(json.dumps(result, indent=2)); return 2
    if not token:
        result = {"status": "BLOCKED_TL2_AUTH", "promotion": None, "identity": TL2_ID, "address": host, "reason": "KEX_BEARER_TOKEN is required for non-loopback TL2 action-runtime binding"}
        _write_report(result); _proof("TL2_AUTH_BLOCKED", result); print(json.dumps(result, indent=2)); return 5
    ...
    with socket.socket() as sock:
        try:
            ...
            _write_report(result); _proof("TL2_BIND_BLOCKED", result); print(json.dumps(result, indent=2)); return 3
    ...
    supervisor_state = _wait_for_current_generation(supervisor)
    generation_owned = (
        supervisor.poll() is None
        and supervisor_state.get("supervisorPid") == supervisor.pid
        and supervisor_state.get("observedState") == "RUNNING"
        and _pid_alive(supervisor_state.get("childPid"))
    )
    ...
    ok = generation_owned and checks and all(check["ok"] for check in checks)
    result = {
        ...
        "supervision": {
            ...
            "aliveAfterReadback": supervisor.poll() is None,
            "currentGenerationOwned": generation_owned,
        ...
        "promotionInvariant": "TL2_LIVE requires the newly spawned supervisor PID, RUNNING state, live child PID, and authenticated inside-tunnel readbacks.",
    }
    _write_report(result); _proof("TL2_DEPLOYMENT_READBACK", result); print(json.dumps(result, indent=2))
