...
class SecretResolver:
    ...
    def resolve(self, capability: str) -> dict[str, str]:
        ...
        proc = subprocess.run([self.command, "resolve", capability], text=True, capture_output=True, timeout=10, check=False, env={"PATH": os.environ.get("PATH", "/usr/bin:/bin")})
...
def _stripe_request(secret_key: str, path: str, form: dict[str, Any]) -> dict[str, Any]:
    ...
    req = urllib.request.Request("https://api.stripe.com" + path, data=encoded, method="POST", headers={"Authorization": f"Bearer {secret_key}", "Content-Type": "application/x-www-form-urlencoded", "User-Agent": "KEX-Capability-Runner/1.1"})
    try:
...
def _checkout_form(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    admission_id = str(payload.get("admission_id") or "").strip()
    tenant_id = str(payload.get("tenant_id") or "").strip()
    system_id = str(payload.get("system_id") or "").strip()
    service_id = str(payload.get("service_id") or "").strip()
    if not admission_id or not tenant_id or not system_id or not service_id:
    ...
    form: dict[str, Any] = {
        ...
        "metadata[admission_id]": admission_id[:500],
...
def _sanitize_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    safe_obj = {k: obj.get(k) for k in ("id", "object", "customer", "subscription", "payment_status", "status", "mode", "client_reference_id", "metadata") if k in obj}
    return {"id": event.get("id"), "type": event.get("type"), "created": event.get("created"), "livemode": event.get("livemode"), "data": {"object": safe_obj}}
...
class KEXCapabilityRunner:
    def __init__(self, *, resolver: SecretResolver | Any | None = None, stripe_transport: Callable[[str, str, dict[str, Any]], dict[str, Any]] = _stripe_request, authority_check: Callable[[dict[str, Any]], bool] | None = None, ledger_path: str | Path | None = None):
        self.resolver = resolver or SecretResolver(); self.stripe_transport = stripe_transport
    ...
    def _receipt(self, request: dict[str, Any], status: str, error: str | None = None) -> str:
        receipt = {"schema": "kex.capability-execution.receipt.v1", "receipt_id": f"KEXCAP-{uuid.uuid4().hex[:16]}", "timestamp": time.time(), "status": status, "capability": request.get("capability"), "operation": request.get("operation"), "caller": request.get("caller"), "result_policy": request.get("result_policy"), "error": error, "secret_material_returned": False}
        try:
            self.ledger_path.parent.mkdir(parents=True, exist_ok=True)
            with self.ledger_path.open("a", encoding="utf-8") as f: f.write(json.dumps(receipt, sort_keys=True) + "\n")
        except OSError:
            pass
        return receipt["receipt_id"]
    def execute(self, request: dict[str, Any]) -> dict[str, Any]:
        if request.get("op") != "EXECUTE_CAPABILITY": return {"status": "REJECTED", "error": "UNKNOWN_OPERATION"}
        if request.get("capability") != PAYMENT_CAPABILITY: return {"status": "REJECTED", "error": "CAPABILITY_NOT_REGISTERED"}
        operation = str(request.get("operation") or "")
        if operation not in ALLOWED_OPERATIONS: return {"status": "REJECTED", "error": "OPERATION_NOT_AUTHORIZED"}
        if request.get("result_policy") != RESULT_POLICY: return {"status": "REJECTED", "error": "RESULT_POLICY_REQUIRED"}
        if not self.authority_check(request): return {"status": "REJECTED", "error": "CALLER_NOT_AUTHORIZED"}
        try:
            secrets = self.resolver.resolve(PAYMENT_CAPABILITY); payload = request.get("payload") or {}
            if operation == "STRIPE_CREATE_CHECKOUT":
                key = secrets.get("secret_key") or secrets.get("stripe_secret_key")
                if not key: raise CapabilityError("STRIPE_SECRET_KEY_UNAVAILABLE")
                provider = self.stripe_transport(key, "/v1/checkout/sessions", _checkout_form(payload))
                session_id, checkout_url = provider.get("id"), provider.get("url")
                if not session_id or not checkout_url: raise CapabilityError("STRIPE_CHECKOUT_RESPONSE_INCOMPLETE")
                ...
                if not whsec: raise CapabilityError("STRIPE_WEBHOOK_SECRET_UNAVAILABLE")
                try: raw = base64.b64decode(str(payload.get("payload_b64") or ""), validate=True)
                except Exception as exc: raise CapabilityError("STRIPE_WEBHOOK_PAYLOAD_INVALID") from exc
                _verify_stripe_signature(raw, str(payload.get("stripe_signature") or ""), whsec)
                try: event = json.loads(raw.decode("utf-8"))
                except Exception as exc: raise CapabilityError("STRIPE_WEBHOOK_JSON_INVALID") from exc
                if not isinstance(event, dict) or not event.get("id") or not event.get("type"): raise CapabilityError("STRIPE_WEBHOOK_EVENT_INVALID")
            ...
            receipt_id = self._receipt(request, "REJECTED", str(exc)); return {"status": "REJECTED", "receipt_id": receipt_id, "error": str(exc)}
        except Exception:
            receipt_id = self._receipt(request, "REJECTED", "CAPABILITY_EXECUTION_FAILED"); return {"status": "REJECTED", "receipt_id": receipt_id, "error": "CAPABILITY_EXECUTION_FAILED"}
...
def _recv_line(conn: socket.socket) -> bytes:
    ...
    while not data.endswith(b"\n"):
        ...
        if not chunk: break
        data += chunk
        if len(data) > MAX_MESSAGE: raise CapabilityError("REQUEST_TOO_LARGE")
    return data

def serve(socket_path: str | None = None) -> None:
    path = Path(socket_path or os.environ.get("KEX_RUNNER_SOCKET", DEFAULT_SOCKET)); path.parent.mkdir(parents=True, exist_ok=True)
    try: path.unlink()
    except FileNotFoundError: pass
    server = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM); server.bind(str(path)); os.chmod(path, int(os.environ.get("KEX_RUNNER_SOCKET_MODE", "660"), 8)); server.listen(32)
    ...
    try:
        while True:
            ...
            with conn:
                try:
                    raw = _recv_line(conn); request = json.loads(raw.decode("utf-8")) if raw else {}; response = runner.execute(request if isinstance(request, dict) else {})
                except Exception:
