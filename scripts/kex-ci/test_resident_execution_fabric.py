...
class ResidentExecutionFabricTests(unittest.TestCase):
    ...
    def tearDown(self):
        try:
            for spec in list(DEFAULT_ROUTES.values()):
                proc=self.fabric._processes.get(spec["runtime_id"])
                if proc:proc.stop(timeout=0.2)
        finally:
            DEFAULT_ROUTES.clear(); DEFAULT_ROUTES.update(self.routes_before); self.tmp.cleanup()
    ...
    def test_same_signed_envelope_cannot_replay(self):
        ...
        with self.assertRaises(ReplayError):self.fabric.dispatch(envelope)
    def test_stale_epoch_is_fenced_even_with_fresh_nonce(self):
        self.fabric.dispatch(self.envelope("work-epoch",epoch=1)); fresh=self.envelope("work-epoch",epoch=1)
        with self.assertRaises(StaleEpochError):self.fabric.dispatch(fresh)
    ...
    def test_managed_runtime_lifecycle_and_restart_accounting(self):
        started=self.fabric.dispatch(self.envelope("work-start",route="test-service",operation="START_RUNTIME")); self.assertTrue(started["observed"]["alive"]); self.assertEqual(started["observed"]["generation"],1)
        restarted=self.fabric.dispatch(self.envelope("work-restart",route="test-service",operation="RESTART_RUNTIME")); self.assertTrue(restarted["observed"]["alive"]); self.assertEqual(restarted["observed"]["generation"],2); self.assertEqual(restarted["observed"]["restart_count"],1)
        readback=self.fabric.dispatch(self.envelope("work-readback",route="test-service",operation="READBACK_RUNTIME")); self.assertTrue(readback["observed"]["process"]["alive"])
        stopped=self.fabric.dispatch(self.envelope("work-stop",route="test-service",operation="STOP_RUNTIME")); self.assertFalse(stopped["observed"]["alive"])
        registered=self.fabric.registry.get("runtime://test/service"); self.assertEqual(registered["desired_state"],"STOPPED"); self.assertEqual(registered["observed_state"],"STOPPED"); self.assertEqual(registered["restart_count"],1)
    def test_envelope_tamper_invalidates_root_before_signature_is_trusted(self):
        envelope=self.envelope("work-tamper"); envelope["operation"]="DESCRIBE_ROUTE"
        with self.assertRaises(Exception) as ctx:self.fabric.dispatch(envelope)
