...
def integrity_readback(manifest_path:Path,integrity_path:Path)->Dict[str,Any]:
    try:
        raw=manifest_path.read_bytes(); observed=hashlib.sha256(raw).hexdigest(); text=integrity_path.read_text(encoding="utf-8").strip(); expected=text.split()[0]; parsed=json.loads(raw.decode("utf-8"))
    except Exception as exc:
        return {"valid":False,"reason":"readback_error:"+type(exc).__name__,"manifest_path":str(manifest_path),"integrity_path":str(integrity_path)}
    valid=bool(parsed.get("applicationId")) and bool(parsed.get("version")) and observed==expected
    return {"valid":valid,"reason":"verified" if valid else "manifest_or_hash_mismatch","expected_sha256":expected,"observed_sha256":observed,"application_id":parsed.get("applicationId"),"version":parsed.get("version"),"manifest_path":str(manifest_path)}
def registry_paths(root:Path)->List[Path]:
    config=root/"config"; primary=config/"kapp_product_registry.json"; extensions=sorted(p for p in config.glob("kapp_*_registry.json") if p!=primary); return [primary,*extensions]
def iter_products(root:Path)->Iterable[Dict[str,Any]]:
    ...
    for registry in registry_paths(root):
        ...
        for product in payload.get("products",[]):
            ...
            if product_id in seen: raise ValueError("duplicate_product:"+str(product_id))
            seen.add(product_id); yield product
...
def build_manifest(root:Path,p:Dict[str,Any])->tuple[Path,str]:
    ...
    mp=d/"k-app.manifest.json"; raw=canonical_bytes(m); mp.write_bytes(raw); h=hashlib.sha256(raw).hexdigest(); (d/"integrity.sha256").write_text(h+"  k-app.manifest.json\n",encoding="utf-8"); return mp,h
def run_product(root:Path,pid:str,emit_receipt:bool=False)->Dict[str,Any]:
    root=root.expanduser().resolve(); p=load_product(root,pid); modules=all((root/x).is_file() for x in p["modules"]); entry=(root/p["entrypoint"]).is_file(); mp,h=build_manifest(root,p); integrity=integrity_readback(mp,root/"runtime_volume"/"kapps"/pid/"integrity.sha256"); integrity_ok=bool(integrity.get("valid"))
