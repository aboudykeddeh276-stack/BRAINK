...
class KEXSymbolicBridgeTests(unittest.TestCase):
    ...
    def test_symbolic_envelope_is_deterministic(self):
        ...
        a = bind_symbol(
            d,
            concept_id="concept:cpu.activate",
            symbol_id="illlm:op:activate",
    ...
    def test_binary_boundary_roundtrip_restores_semantics(self):
        e = bind_symbol(
            dictionary(),
            concept_id="concept:router",
            symbol_id="illlm:noun:router",
            attributes={"route": "mesh0"},
        )
    ...
    def test_binary_tamper_fails_root_check(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "KEX_BINARY_BOUNDARY_PACKED_HASH_MISMATCH",
        ):
    ...
    def test_linguistic_projection_binding_preserves_source_root(self):
        projection = {
            ...
            "projection": {
                "language": "en",
                "text": "strong reactive Oxygen",
            },
            "truth_boundary": {"projection_mutates_source": False},
            "semantic_root": "abc123",
        }
        e = from_linguistic_projection(
            dictionary(),
            projection,
            concept_id="concept:oxygen.activate",
            symbol_id="illlm:op:activate",
        )
    ...
    def test_linguistic_projection_source_mutation_is_rejected(self):
        projection = {
            "semantic_root": "abc",
            "truth_boundary": {"projection_mutates_source": True},
        }
        with self.assertRaisesRegex(
            ValueError,
            "ILLLM_SOURCE_MUTATION_BOUNDARY_INVALID",
        ):
            from_linguistic_projection(
                dictionary(),
                projection,
