...
def authorize_url(cfg,domain,return_to):
    ...
    redirect=cfg["redirect_uris"][domain]
    _states[state]={"verifier":verifier,"domain":domain,"return_to":return_to,"exp":time.time()+600}
...
def callback(cfg,q):
    state=q.get("state"); code=q.get("code"); st=_states.pop(state,None)
    ...
    req=urllib.request.Request(cfg.get("userinfo_endpoint","https://openidconnect.googleapis.com/v1/userinfo"),headers={"Authorization":"Bearer "+tok["access_token"]})
    profile=json.loads(urllib.request.urlopen(req,timeout=15).read())
    return {"session":secrets.token_urlsafe(32),"profile":{"sub":profile.get("sub"),"email":profile.get("email"),"name":profile.get("name"),"picture":profile.get("picture")},"return_to":st["return_to"]}
