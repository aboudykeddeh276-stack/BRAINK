...
def stripe_post(path, fields, secret):
    ...
    req=urllib.request.Request("https://api.stripe.com"+path,data=data,headers={"Authorization":"Bearer "+secret,"Content-Type":"application/x-www-form-urlencoded"})
    return json.loads(urllib.request.urlopen(req,timeout=20).read())

def checkout(cg, req):
    domain=req.get("domain","braink.com.au"); product=req.get("product","BRAINK")
    price=cg["prices"].get(product) or cg["prices"]["BRAINK"]
...
def handle(c):
    ...
    if op=="WEBHOOK":
        ev=verify_webhook(cg,base64.b64decode(req["payload_b64"]),req.get("stripe_signature",""))
        return reply(c,{"status":"PASS","event":{"id":ev.get("id"),"type":ev.get("type")}})
