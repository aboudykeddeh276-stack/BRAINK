...
SECTORS = {
    "MACHINE_RUNTIME": {
        "requires": ["medium_root","controller_root","braink_root","lineage_root"],
        "effect": "machine boots controller-first; BRAINK resident/root object is decoded from machine medium",
    },
    "STORAGE": {
        "requires": ["encoded_medium","zeroless_geometry","controller"],
        "effect": "capacity/addressability derives from encoded geometry/controller contract, never source-line or sheet count",
    },
    "VFS": {
        "requires": ["role"],
        "effect": "VFS remains resolver/namespace only",
    },
    "MEMORY": {
        "requires": ["address_geometry","controller_binding"],
        "effect": "memory arrays are encoded/addressed state under controller semantics; workbook rows remain projections",
    },
    "NETWORK": {
        "requires": ["lexical_id","vector_id","carrier"],
        "effect": "BRAINK lexical/vector identity precedes IP/HTTP/QUIC carrier mapping",
    },
    "CLOUD": {
        "requires": ["canonical_object_id","replica_set","lineage"],
        "effect": "cloud is replicated machine-resident object state with stable canonical identity and distinct machine lineage",
    },
    "DOMAIN": {
        "requires": ["domain_object","lexical_id","server_root","storage_root"],
        "effect": "domain is a resident typed object; public DNS/registrar/TLS are adapter boundaries",
    },
    "DNS": {
        "requires": ["internal_root","authority_state"],
        "effect": "internal DNS state may mutate locally; public DNS promotion requires external authoritative readback/mutation",
    },
    "REGISTRAR": {
        "requires": ["internal_root","authority_state"],
        "effect": "registrar state is resident/internal; registry authority remains an external adapter boundary",
    },
    "TLS": {
        "requires": ["internal_root","authority_state"],
        "effect": "TLS intent/state is resident/internal; CA-issued state requires external certificate proof",
    },
    "AGENTS": {
        "requires": ["braink_id","machine_id","lineage_root"],
        "effect": "agents inherit machine/BRAINK identity and storage/controller roots rather than floating as unattached services",
    },
    "OBSERVER_PROOF": {
        "requires": ["observer_root","proof_root"],
        "effect": "workbook and receipts observe/project state; they do not manufacture substrate capacity or authority",
    },
    "DEPLOYMENT": {
        "requires": ["promotion_gate","readback"],
        "effect": "deployment status promotes only after runtime mutation and readback on the relevant authority boundary",
    },
}
...
def main():
    ...
    receipt={
      "schema":"braink.sector-impact.r16",
      "authoritative_chain":AUTHORITATIVE_CHAIN,
      "sector_results":results,
      "negative_guard_test":asdict(negative),
      "all_sector_contracts_pass":all(x["status"]=="PASS" for x in results),
      "forbidden_semantics_rejected":negative.status=="FAIL" and "VFS_IS_STORAGE_MEDIUM" in negative.violations,
    }
...
if __name__=="__main__":
    r=main()
    out=sys.argv[1] if len(sys.argv)>1 else "/mnt/data/BRAINK_R16_SECTOR_IMPACT_RECEIPT.json"
