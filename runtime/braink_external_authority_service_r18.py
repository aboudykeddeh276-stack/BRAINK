...
PORT = int(os.environ.get('BRAINK_EXTERNAL_PORT', '17942'))
LEDGER = Path(os.environ.get('BRAINK_EXTERNAL_LEDGER', '/mnt/data/BRAINK_EXTERNAL_AUTHORITY_SERVICE_R18_LEDGER.jsonl'))
STATE = Path(os.environ.get('BRAINK_EXTERNAL_STATE', '/mnt/data/BRAINK_EXTERNAL_AUTHORITY_SERVICE_R18_STATE.json'))

IDENTITY = {
    'service_id':'BRAINK::SERVICE::EXTERNAL_AUTHORITY::R18',
...
def https_probe(domain):
    try:
        req=urllib.request.Request('https://'+domain+'/',method='HEAD',headers={'User-Agent':'BRAINK-ExternalAuthority-R18/1.0'})
...
def rdap_probe(domain):
    ...
    try:
        req=urllib.request.Request(url,headers={'User-Agent':'BRAINK-ExternalAuthority-R18/1.0','Accept':'application/rdap+json,application/json'})
...
def observe():
    ...
    receipt={
        'schema':'braink.external-authority-service.r18.observation','time':now(),'identity':IDENTITY,'domain':DOMAIN,
