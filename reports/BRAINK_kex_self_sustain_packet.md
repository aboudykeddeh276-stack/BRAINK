...
- `auth.oauth`: MODEL-LOCAL
- `chrome_browser`: MODEL-LOCAL
- `constraint_flags`: MODEL-LOCAL
- `general`: MODEL-LOCAL
- `illlm_bootstrap`: MODEL-LOCAL
- `illlm_bundle`: MODEL-LOCAL
- `illlm_query`: MODEL-LOCAL
- `inner_runtime`: MODEL-LOCAL
- `module_manifest`: MODEL-LOCAL
- `proof_packet`: MODEL-LOCAL
- `runtime_trace`: MODEL-LOCAL
- `scrape_tool`: MODEL-LOCAL

## Pending tasks

- **A_PORTABLE_ROOTS** [PENDING]: Replace host-only proof/report paths with configurable BRAINK_ROOT and BRAINK_BUILD_DIR providers. Proof: Unit/static check proving no required output path depends on /Users/ak.
- **B_SELF_SUSTAIN_ORCHESTRATOR** [COMPLETED]: Use this tool to generate per-repo manifest and task packets before coding actions. Proof: JSON packet with SHA-256 artifacts and allowed status ledger.
- **C_ROUTE_PROOF** [PENDING]: Add route-by-route assertions for classifier, resolver, payload, audit row, and smoke marker. Proof: Executable route smoke report for every README route token.
- **D_MANIFEST_HASHES** [PENDING]: Promote generated artifact hashes into a stable manifest verification gate. Proof: Manifest checker exits non-zero on changed hashes or stale counters.
- **E_KEX_ETHICS** [MODEL-LOCAL]: Promote KEX affect/bioethics predicates into an executable checker and report gate. Proof: Checker report with no unsupported medical/body/sentience claims.
- **F_KEX_REPO_BINDING** [PENDING]: Define and test a fixture KEX hyperdrive repository adapter for theorem-lineage metadata. Proof: Fixture repo produces anchor/theorem/constraint/action/proof/status packet.
- **G_MACOS_RUNTIME_PROOF** [BLOCKED]: Run SwiftUI/AppKit build and runtime smoke in a macOS environment. Proof: SMOKE_STATUS:DONE plus generated JSON proof artifacts.
- **H_EXTERNAL_BOUNDARY** [EXTERNALLY-UNVALIDATED]: Require independent reproduction packages before promoting external scientific/hardware claims. Proof: External source-backed validation package, measurements, or reproduction logs.
